...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	while (curr != NULL){
		found = compare(curr->val, val);
		// value found
		if (found == 0)
			return 1;
		// current values is smaller so look to the right
		else if (found == -1)
			curr = curr->right;
		// current values is larger so look to the left
		else
			curr = curr->left;
	}
	// if the we reach a null node
	return 0;
}
